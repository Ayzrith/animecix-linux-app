...
pub fn config_candidates(home: &str, bin: &PathBuf) -> Vec<PathBuf> {
    ...
    v.push(PathBuf::from(format!(
        "{home}/.local/share/singbox/config.json"
    )));
...
pub fn start(bin: &PathBuf, cfg: &PathBuf) -> Result<(), String> {
    ...
    let mut logf = std::fs::OpenOptions::new()
        ...
        .append(true)
        .open(&log)
        .map_err(|e| format!("log açılamadı ({}): {e}", log.display()))?;
...
mod tests {
    ...
    fn default_candidate_paths_cover_known_locations() {
        ...
        assert_eq!(
            bins[0],
            PathBuf::from("/home/x/.local/share/singbox/sing-box")
        );
        assert!(bins
            .iter()
            .any(|b| *b == PathBuf::from("/usr/bin/sing-box")));
        let cfgs = config_candidates("/home/x", &bins[0]);
        assert_eq!(
            cfgs[0],
            PathBuf::from("/home/x/.local/share/singbox/config.json")
        );
