...
pub struct App {
    ...
    pub rec_btn: gtk::Button,
    pub search_toggle_btn: gtk::Button,
    pub sidebar: gtk::Revealer,
    pub fs_bar: gtk::Box,
    ...
    pub fullscreen_mode: Rc<Cell<bool>>,
}
...
pub(crate) fn resolve_upscale_shader(name: &str) -> Option<String> {
    ...
    fn embedded(name: &str) -> Option<&'static str> {
        match name {
            "Anime4K_Upscale_CNN_x2_M.glsl" => Some(include_str!(
                "../assets/upscale/Anime4K_Upscale_CNN_x2_M.glsl"
            )),
            "Anime4K_Upscale_CNN_x2_UL.glsl" => Some(include_str!(
                "../assets/upscale/Anime4K_Upscale_CNN_x2_UL.glsl"
            )),
            "Anime4K_Upscale_DTD_x2.glsl" => Some(include_str!(
                "../assets/upscale/Anime4K_Upscale_DTD_x2.glsl"
            )),
            "Anime4K_Upscale_Original_x2.glsl" => Some(include_str!(
                "../assets/upscale/Anime4K_Upscale_Original_x2.glsl"
            )),
            _ => None,
        }
    }
        ...
        if !ad.is_empty() {
            candidates.push(
                std::path::Path::new(&ad)
                    .join("usr/share/animecix/assets/upscale")
                    .join(name),
            );
        }
    }
...
fn aniskip_input_conf(t: &api::AniSkipTimes) -> String {
    ...
    let skip_cmd = if let (Some(st), Some(et)) = (t.op_start, t.op_end) {
        format!(
            "s seek {et:.1} absolute; show-text \"⏩ İntro Atlandı (AniSkip: {} → {})\" 3000\n",
            fmt_sec(st),
            fmt_sec(et)
        )
    } else {
    ...
    let outro_cmd = if let (Some(st), Some(et)) = (t.ed_start, t.ed_end) {
        format!(
            "e seek {et:.1} absolute; show-text \"⏩ Outro Atlandı (AniSkip: {} → {})\" 3000\n",
            fmt_sec(st),
            fmt_sec(et)
        )
    } else {
...
impl App {
    pub fn new(app: &adw::Application) -> Rc<Self> {
        ...
        header.add_css_class("custom-titlebar");
        ...
        menu_btn.set_tooltip_text(Some("Menü (soldan açılır panel)"));
        header.pack_start(&menu_btn);

        let back_btn = gtk::Button::from_icon_name("go-previous-symbolic");
        back_btn.add_css_class("flat");
        back_btn.add_css_class("circular");
        back_btn.set_tooltip_text(Some("Geri (Esc / Alt+← / Fare Geri Tuşu)"));
        ...
        search_toggle_btn.set_tooltip_text(Some("Arama Yap (Ctrl+S)"));
        header.pack_start(&search_toggle_btn);

        let right_box = gtk::Box::new(gtk::Orientation::Horizontal, 2);
        right_box.add_css_class("titlebar-rightbox");
        ...
        min_btn.add_css_class("titlebar-win-btn");
        min_btn.set_tooltip_text(Some("Küçült"));
        ...
        max_btn.add_css_class("circular");
        max_btn.add_css_class("titlebar-win-btn");
        max_btn.set_tooltip_text(Some("Büyüt / Geri Yükle"));
        ...
        close_btn.add_css_class("custom-close-btn");
        close_btn.add_css_class("titlebar-win-btn");
        close_btn.set_tooltip_text(Some("Kapat"));
        close_btn.set_valign(gtk::Align::Center);
        right_box.append(&close_btn);

        header.pack_end(&right_box);

        fn sidebar_button(icon: &str, label: &str, tooltip: &str) -> gtk::Button {
            let b = gtk::Button::new();
            b.add_css_class("flat");
            b.add_css_class("sidebar-nav-btn");
            ...
            img.add_css_class("sidebar-nav-icon");
        ...
        let home_btn = sidebar_button("go-home-symbolic", "Ana Sayfa", "Ana Sayfa");
        let rec_btn = sidebar_button(
            "emblem-favorite-symbolic",
            "Öneriler",
            "Sana özel anime önerileri",
        );
        let fav_btn = sidebar_button("starred-symbolic", "Favoriler", "Favorilerim");
        let marathon_btn = sidebar_button(
            "media-playlist-repeat-symbolic",
            "Maraton",
            "İzleme Maratonum",
        );
        let hist_btn = sidebar_button("document-open-recent-symbolic", "Geçmiş", "İzleme Geçmişi");
        let settings_btn = sidebar_button("emblem-system-symbolic", "Ayarlar", "Ayarlar");
        ...
        let side_title = gtk::Label::new(Some("AnimeciX"));
        side_title.add_css_class("title-3");
        side_title.add_css_class("sidebar-title");
        side_title.set_xalign(0.0);
        side_title.set_margin_start(12);
        side_title.set_margin_bottom(8);
        sidebar_box.append(&side_title);
        sidebar_box.append(&home_btn);
        sidebar_box.append(&rec_btn);
        sidebar_box.append(&fav_btn);
        sidebar_box.append(&marathon_btn);
        sidebar_box.append(&hist_btn);
        sidebar_box.append(&settings_btn);

        let sidebar_frame = gtk::Box::new(gtk::Orientation::Vertical, 0);
        sidebar_frame.add_css_class("sidebar-frame");
        sidebar_frame.set_size_request(240, -1);
        sidebar_frame.append(&sidebar_box);

        let sidebar = gtk::Revealer::new();
        sidebar.set_transition_type(gtk::RevealerTransitionType::SlideRight);
        sidebar.set_transition_duration(250);
        sidebar.set_reveal_child(false);
        sidebar.set_child(Some(&sidebar_frame));
        ...
        let fs_bar = gtk::Box::new(gtk::Orientation::Horizontal, 4);
        fs_bar.set_visible(false);

        let fs_menu_btn = gtk::Button::from_icon_name("open-menu-symbolic");
        let fs_back_btn = gtk::Button::from_icon_name("go-previous-symbolic");
        let fs_search_btn = gtk::Button::from_icon_name("system-search-symbolic");
        let fs_exit_btn = gtk::Button::from_icon_name("view-restore-symbolic");

        let content = gtk::Box::new(gtk::Orientation::Vertical, 0);

        let title_handle = gtk::WindowHandle::new();
        title_handle.set_child(Some(&header));
        content.append(&title_handle);
        ...
        let initial_page = if welcome_seen {
            Page::Home
        } else {
            Page::Welcome
        };
        ...
        let app_inst = Rc::new(Self {
            ...
            fullscreen_mode: Rc::new(Cell::new(false)),
        });
        {}

        {
            let a = app_inst.clone();
            fs_menu_btn.connect_clicked(move |_| {
                let open = !a.sidebar.reveals_child();
                a.sidebar.set_reveal_child(open);
            });
            let a = app_inst.clone();
            fs_back_btn.connect_clicked(move |_| a.go_back());
            let a = app_inst.clone();
            fs_search_btn.connect_clicked(move |_| {
                a.search_bar.set_search_mode(true);
                a.search_entry.grab_focus();
            });
            let a = app_inst.clone();
            fs_exit_btn.connect_clicked(move |_| a.toggle_fullscreen());
        }

        {
            let w = app_inst.window.clone();
            ...
            max_btn.connect_clicked(move |_| {
                if w.is_maximized() {
                    w.unmaximize();
                    max_c.set_icon_name("window-maximize-symbolic");
                } else {
                    w.maximize();
                    max_c.set_icon_name("window-restore-symbolic");
                }
            });
            let w = app_inst.window.clone();
            let max_c2 = max_btn.clone();
            w.connect_maximized_notify(move |ww| {
                if ww.is_maximized() {
                    max_c2.set_icon_name("window-restore-symbolic");
                } else {
                    max_c2.set_icon_name("window-maximize-symbolic");
                }
            });
            let w = app_inst.window.clone();
    ...
    pub fn clone_ref(&self) -> Rc<Self> {
        Rc::new(Self {
            ...
            menu_btn: self.menu_btn.clone(),
            home_btn: self.home_btn.clone(),
            rec_btn: self.rec_btn.clone(),
            search_toggle_btn: self.search_toggle_btn.clone(),
            sidebar: self.sidebar.clone(),
            fs_bar: self.fs_bar.clone(),
            fs_exit_btn: self.fs_exit_btn.clone(),
            ...
            fullscreen_mode: self.fullscreen_mode.clone(),
        })
    }

    pub fn go_back(&self) {
        if self.sidebar.reveals_child() {
    ...
    pub fn toggle_fullscreen(&self) {
        if self.window.is_fullscreened() {
    ...
    pub fn refill_for_fullscreen(&self) {
        let top = self
            .page_history
            .borrow()
            .last()
        ...
        match &top {
            Page::Home | Page::Search => {
                ...
                if self.fullscreen_mode.get() {
                    ...
                    let posters: Vec<String> = self
                        .cats
                        .borrow()
                        .iter()
                        .flat_map(|c| c.items.iter())
                        .take(40)
                        .filter_map(|t| t.poster.clone())
                        .collect();
                    std::thread::spawn(move || {
                        for url in posters.into_iter() {
                            let hd_url = CoverManager::thumb_url(&url, true);
                            let _ = client.get_bytes(&hd_url);
                        }
                    });
                }
            }
    ...
    fn chain_signals(&self) {
        ...
        self.menu_btn.connect_clicked(move |_| {
            let open = !this.sidebar.reveals_child();
            this.sidebar.set_reveal_child(open);
        });

        let sidebar_c = self.sidebar.clone();
        let this_fs = self.clone_ref();
        self.window.connect_fullscreened_notify(move |w| {
            let fs = w.is_fullscreened();
            this_fs.fullscreen_mode.set(fs);
            if fs {
                sidebar_c.set_reveal_child(false);

                w.add_css_class("is-fullscreen");
            } else {
                w.remove_css_class("is-fullscreen");
            }

            this_fs.refill_for_fullscreen();
        });
        ...
        self.search_entry.connect_activate(move |e| {
            ...
            if !q.trim().is_empty() {
                this.sidebar.set_reveal_child(false);
                this.do_search(q);
            }
        });

        let this = self.clone_ref();
        self.home_btn.connect_clicked(move |_| {
            this.sidebar.set_reveal_child(false);
            let mut st = this.page_history.borrow_mut();
            if st.last() != Some(&Page::Home) {
                st.push(Page::Home);
            }
            drop(st);
        ...
        self.rec_btn.connect_clicked(move |_| {
            this.sidebar.set_reveal_child(false);
            let mut st = this.page_history.borrow_mut();
            if st.last() != Some(&Page::Recommendations) {
                st.push(Page::Recommendations);
            }
            drop(st);
            this.show_page(&Page::Recommendations);
            this.fetch_home();
        });
...

        {
            ...
            key_ctrl.connect_key_pressed(move |_, keyval, _, state| {
                ...
                if is_alt && (key_name == "Left" || key_name == "KP_Left") {
                    this.go_back();
                    return glib::Propagation::Stop;
                }
                ...
                let triggered = match sc.as_str() {
                    ...
                    _ => is_ctrl && (key_name == "s" || key_name == "S"),
                };
                if triggered {
    ...
    fn apply_movie_tint(&self, target: &gtk::Box, poster: Option<&str>) {
        let Some(url) = poster.map(|s| s.to_string()) else {
            return;
        };
        let client = self.client.clone();
        ...
        glib::idle_add_local(move || match rx.try_recv() {
            Ok(pal) => {
                ...
                let [c1, c2, c3] = pal.unwrap_or([(122, 162, 247), (55, 70, 110), (140, 110, 190)]);
                let (r1, g1, b1) = c1;
                let (r2, g2, b2) = c2;
                let (r3, g3, b3) = c3;
                let css_a = format!(
    ...
    pub fn show_page(&self, page: &Page) {
        ...
        self.back_btn
            .set_sensitive(self.page_history.borrow().len() > 1);
        ...
        fn switch<T: IsA<gtk::Widget>>(
            ...
            glib::timeout_add_local_once(std::time::Duration::from_millis(400), move || {
                let Some(visible) = stack_c.visible_child() else {
                    return;
                };
                let mut to_rm = vec![];
        ...
        match page {
            ...
            Page::Recommendations => {
                self.title_label.set_text("Öneriler ✨");
                switch(
                    &self.stack,
                    "recommendations",
                    gtk::StackTransitionType::Crossfade,
                    self.build_recommendations_view(),
                );
            }
            Page::Favs => {
            ...
            Page::Episodes { title, eps } | Page::Movie { title, eps } => {
                ...
                switch(
                    ...
                    &page_name,
                    gtk::StackTransitionType::SlideLeft,
                    self.build_episodes_view(title, eps),
                );
            }
        }
    }
    ...
    fn goto_page(&self, val: &str) {
        ...
        match val {
            ...
            "recommendations" => {
                self.page_history.borrow_mut().push(Page::Recommendations);
                self.show_page(&Page::Recommendations);
                self.fetch_home();
            }
            "favorites" => {
    ...
    fn build_welcome_view(&self) -> gtk::ScrolledWindow {
        ...
        dep_group.set_description(Some(
            "Uygulamanın sorunsuz çalışabilmesi için gerekli sistem araçları:",
        ));
        ...
        for dep in &deps {
            ...
            if !dep.installed {
                if let Some(cmd) = &dep.install_cmd {
                    ...
                    cmd_row.set_title("💻 Kurulum komutu (terminale yapıştırın):");
                    ...
                    copy_btn.set_tooltip_text(Some("Komutu panoya kopyala"));

                    let cmd_clone = cmd.clone();
                    let this_copy = self.clone_ref();
                    copy_btn.connect_clicked(move |_| {
                        if let Some(display) = gtk::gdk::Display::default() {
                            display.clipboard().set_text(&cmd_clone);
                        }
                        let toast = adw::Toast::new(
                            "📋 Komut kopyalandı — terminale yapıştırıp Enter'a basın",
                        );
                        toast.set_timeout(4);
                        this_copy.toast.add_toast(toast);
                    });
        ...
        if is_installed {
            desktop_status_lbl
                .set_markup("<span foreground='#2ec27e' weight='bold'>🟢 Menüde Ekli</span>");
            ...
            desktop_status_lbl
                .set_markup("<span foreground='#f5c211' weight='bold'>🟡 Menüde Yok</span>");
        ...
        desktop_btn.connect_clicked(move |_| match install_desktop_entry() {
            Ok(_) => {
                lbl_clone.set_markup(
                    "<span foreground='#2ec27e' weight='bold'>🟢 Başarıyla Eklendi</span>",
                );
        ...
        aniskip_row.set_subtitle(
            "AniSkip API üzerinden 's' kısayol tuşu ile intro bitişine otomatik atlar",
        );
    ...
    fn create_title_card(&self, t: &Title) -> gtk::Box {
        let fs = self.fullscreen_mode.get();
        let (cw, ch) = if fs { (160, 240) } else { (140, 210) };

        let box_ = gtk::Box::new(gtk::Orientation::Vertical, 2);
        box_.add_css_class("title-btn");
        box_.set_size_request(cw, -1);
        box_.set_hexpand(false);
        box_.set_vexpand(false);
        box_.set_halign(gtk::Align::Center);
        box_.set_valign(gtk::Align::Start);

        let pic = self.covers.cover_picture(t.poster.as_deref(), cw, ch);
        pic.set_size_request(cw, ch);
    ...
    fn build_home_view(&self) -> gtk::ScrolledWindow {
        ...
        let main_box = gtk::Box::new(gtk::Orientation::Vertical, 14);
        ...
        let this = self.clone_ref();
        let make_card = Rc::new(move |t: &Title| this.create_title_card(t));
        for cat in cats.iter() {
            main_box.append(&views::build_shelf(
                &cat.name,
                &cat.items,
                make_card.clone(),
            ));
    ...
    fn build_marathon_view(&self) -> gtk::ScrolledWindow {
        ...
        let view = views::MarathonView::build(
            ...
            move |id| {
                let item = this_toggle
                    .client
                    .get_marathon()
                    .into_iter()
                    .find(|m| m.title.id == id);
                    ...
                    msg => {
                        match msg {
                            Ok(Ok(n)) => {
                                ...
                                let toast = adw::Toast::new(&format!(
                                    "🏁 {n} bölüm izlendi olarak işaretlendi!"
                                ));
                                toast.set_timeout(2);
                            ...
                            _ => {
                                let toast = adw::Toast::new(
                                    "❌ Bölüm listesi alınamadı (internete bağlı mısın?)",
                                );
                                toast.set_timeout(3);
                                this_async.toast.add_toast(toast);
                            }
    ...
    fn build_settings_view(&self) -> gtk::ScrolledWindow {
        ...
        let last_save: Rc<RefCell<std::time::Instant>> =
    ...
    fn build_recommendations_view(&self) -> gtk::ScrolledWindow {
        let cats = self.cats.borrow();
        if cats.is_empty() {
            drop(cats);
            self.fetch_home();
            ...
            let sp = components::create_status_page(
                ...
                "Ana sayfa listeleri çekiliyor, birazdan öneriler burada olacak.",
                "emblem-favorite-symbolic",
            );
            scroll.set_child(Some(&sp));
            return scroll;
        }
        let pool = views::RecommendationsView::pool_from_cats(&cats);
        drop(cats);
        if pool.is_empty() {
            let scroll = gtk::ScrolledWindow::new();
            let sp = components::create_status_page(
                "Öneri Bulunamadı",
                "Şu an öneri havuzu boş. Ana sayfayı yenileyip tekrar deneyin.",
                "emblem-favorite-symbolic",
            );
            scroll.set_child(Some(&sp));
            return scroll;
        ...
        let this = self.clone_ref();
        let make_card = Rc::new(move |t: &Title| this.create_title_card(t));
        views::RecommendationsView::build(pool, make_card)
    }
    ...
    fn build_episodes_view(&self, title: &Title, eps: &[Episode]) -> gtk::ScrolledWindow {
        ...
        let is_movie = title.title_type.as_deref() == Some("movie")
            || (eps.len() <= 1
                && eps
                    .first()
                    .map(|e| e.name.contains("Filmi"))
                    .unwrap_or(false));

        if is_movie {
            let header_poster = self.covers.cover_picture(title.poster.as_deref(), 220, 330);

            header_poster.set_hexpand(false);
            header_poster.set_halign(gtk::Align::Center);
            ...
            bookmark_btn.connect_clicked(move |b| {
                ...
                b.set_icon_name(if saved {
                    "starred-symbolic"
                } else {
                    "non-starred-symbolic"
                });
                b.set_tooltip_text(Some(if saved {
                    "Favorilerden Çıkar"
                } else {
                    "Favorilere Ekle"
                }));
            ...
            marathon_btn.connect_clicked(move |b| {
                ...
                b.set_icon_name(if added {
                    "media-playlist-repeat-symbolic"
                } else {
                    "flag-symbolic"
                });
                b.set_tooltip_text(Some(if added {
                    "Maratondan Çıkar"
                } else {
                    "İzleme Maratonuna Ekle"
                }));
                let msg = if added {
                    "🏆 İzleme Maratonuna eklendi!"
                } else {
                    "İzleme Maratonundan çıkarıldı"
                };
            ...
            self.progress_bars
                .borrow_mut()
                .insert(prog_key, (movie_pb, movie_lbl));
        ...
        header_poster.set_hexpand(false);
        header_poster.set_halign(gtk::Align::Start);
        let bookmark_btn = components::bookmark_button(&self.client, title);
        let this_bm = self.clone_ref();
        let t_clone = title.clone();
        bookmark_btn.connect_clicked(move |b| {
        ...
        let detail_header = episodes_view::create_title_detail_header(
            title,
            &header_poster,
            &bookmark_btn,
            &marathon_btn,
        ...
        if eps.is_empty() {
            ...
            let rows: Vec<(Episode, gtk::Box)> = eps
                .iter()
                .map(|e| {
                    ...
                    self.progress_bars
                        .borrow_mut()
                        .insert(key.clone(), (progress_bar, time_lbl));
            ...
            Msg::Cats(res) => match res {
                Ok(cats) => {
                    *self.cats.borrow_mut() = cats;
                    ...
                    if top == Some(Page::Home) || top == Some(Page::Recommendations) {
    ...
    fn play(&self, title: &Title, ep: &Episode) {
        ...
        let ep = ep.clone();
        eprintln!(
            "[PLAY] çağrıldı: {} S{:02}E{:02}",
            title.name, ep.season, ep.episode
        );
        let is_movie = title.title_type.as_deref() == Some("movie");
        if is_movie {
            self.play_resolved(&title, &ep, None);
    ...
    fn after_fansubs_loaded(
        &self,
        title: Title,
        ep: Episode,
        fansubs: Vec<api::FansubInfo>,
        default_template: Option<i64>,
    ) {
        self.busy(false);
    ...
    fn play_with_fansub(&self, title: &Title, ep: &Episode, fs: &api::FansubInfo) {
        ...
        self.spawn(move |_| {
            let res = client
                ...
                .or_else(|e| {
                    eprintln!("[PLAY-FS] {} mirror'ları çözülemedi: {}", fansub_name, e);
                    Err(format!(
    ...
    fn play_resolved(&self, title: &Title, ep: &Episode, _fansub_template: Option<i64>) {
        let toast = adw::Toast::new(&format!("🎬 {} hazırlanıyor…", title.name));
        toast.set_timeout(2);
        self.toast.add_toast(toast);
        let title = title.clone();
        let ep = ep.clone();
            ...
            let res = if title.title_type.as_deref() == Some("movie") {
                ...
                c.resolve_top(title.id, ep.episode, ep.season, 3, pref.as_deref())
                    .and_then(|fast_pairs| {
                        ...
                        if let Some(p) = &pref {
                            fb.sort_by_key(|u| {
                                if api::Client::source_host_hint(u) == p.as_str() {
                                    0
                                } else {
                                    1
                                }
                        ...
                        let fast: Vec<String> = fast_pairs.iter().map(|(m, _)| m.clone()).collect();
                        let fast_emb: Vec<String> =
                            fast_pairs.iter().map(|(_, e)| e.clone()).collect();
                        Ok((fast, fast_emb, fb))
                    })
            };
    ...
    fn decide_retry(exited: bool, success: bool, playing: bool) -> (bool, bool) {
        if !exited {
    ...
    fn play_candidates(
        ...
        let saved_pos = self
            .client
            .get_progress(tid, season, episode)
        ...
        let aniskip_shared =
            std::sync::Arc::new(std::sync::Mutex::new(api::AniSkipTimes::default()));
        ...
        let t = adw::Toast::new(&format!(
            ...
            saved_pos
                .map(|p| {
                    let s = p as u64;
                    if s >= 3600 {
                        format!(" ({}:{:02}:{:02}'den)", s / 3600, (s % 3600) / 60, s % 60)
                    } else {
                        format!(" ({}:{:02}'den)", s / 60, s % 60)
                    }
                })
                .unwrap_or_default()
        ));
        ...
        if aniskip_enabled {
            ...
            std::thread::spawn(move || {
                ...
                for attempt in 0..MAX_TRIES {
                    if !alive_r.load(std::sync::atomic::Ordering::Relaxed) {
                        return;
                    }
                    ...
                    if t.op_end.is_some() || t.ed_end.is_some() {
                        *shared_r.lock().unwrap() = t;
                        ...
                        let _ = toast_tx_r
                            .send("⏩ AniSkip hazır ('s' ile intro atlarsın)".to_string());
                        return;
                    }
            ...
            std::thread::spawn(move || {
                ...
                loop {
                    if std::path::Path::new(&sock_c).exists() {
                        ...
                        let (pos, dur) =
                            crate::player::query_mpv_position(&sock_c).unwrap_or((0.0, 0.0));
                        ...
                        if let Some(st) = snap.ed_start {
                            if !ed_prompted && pos >= (st - 1.5) && pos <= (st + 25.0) {
                                ed_prompted = true;
                                crate::player::send_mpv_cmd(
                                    &sock_c,
                                    "{\"command\":[\"show-text\", \"🏁 Outro Başladı\", 7000]}\n",
                                );
                            }
                        }
                        if sender.send((pos, dur)).is_err() {
                            break;
                        }
                    } else {
                        if !alive.load(std::sync::atomic::Ordering::Relaxed) {
...

        {
            ...
            std::thread::spawn(move || {
                'supervisor: for i in 0..total {
                    ...
                    eprintln!(
                        "[SUP] mpv spawn deneniyor (ep={}, kaynak={}, url={:.80})",
                        episode, i, url
                    );
                    let child = match cmd.spawn() {
                        ...
                        Err(e) => {
                            eprintln!(
                                "[SUP] HATA mpv başlatılamadı (ep={}, kaynak={}): {}",
                                episode, i, e
                            );
                            continue;
                        }
                    ...
                    *mpv_child_c.lock().unwrap() = Some(child);
                    ...
                    loop {
                        ...
                        if exited {
                            let retry = Self::decide_retry(true, success, playing).0;
                            if retry {
                        ...
                        if std::path::Path::new(&sock_path_c).exists() {
                            if !playing {
                                eprintln!(
                                    "[SUP] socket belirdi, oynatma başladı (ep={}, kaynak={})",
                                    episode, i
                                );
                        ...
                        if playing {
                            let idle = crate::player::query_mpv_prop(&sock_path_c, "core-idle")
                                .unwrap_or(0.0);
                            let dur = crate::player::query_mpv_prop(&sock_path_c, "duration")
                                .unwrap_or(0.0);
                            if dur > 0.0 {
                                media_loaded = true;
                            }
                            let elapsed_secs = start.elapsed().as_secs();
                            if Self::source_is_dead(
                                elapsed_secs,
                                idle >= 1.0,
                                dur,
                                media_loaded,
                                ...
                                let _ = toast_tx_c.send(
                                    "Kaynak açıldı ama oynatamadı, diğer kaynağa geçiliyor…"
                                        .to_string(),
                                );
                    ...
                    eprintln!(
                        "[SUP] döngü bitti (ep={}, kaynak={}, playing={})",
                        episode, i, playing
                    );
                    ...
                    if playing {
                        ...
                        if i + 1 < total {
                            let _ = toast_tx_c
                                .send("Kaynak açılamadı, diğer kaynağa geçiliyor…".to_string());
                            continue;
                        } else {
...
mod decide_retry_tests {
    ...
    fn slow_source_within_window_is_not_killed() {
        assert!(
            !App::source_is_dead(5, true, 0.0, false, 20),
            "5sn'de öldürülmemeli"
        );
        assert!(
            !App::source_is_dead(19, true, 0.0, false, 20),
            "19sn'de hâlâ sabırlı olunmalı"
        );
    }
    ...
    fn never_loaded_idle_source_is_dead_after_threshold() {
        assert!(
            App::source_is_dead(20, true, 0.0, false, 20),
            "20sn+idle+dur=0 -> ölü"
        );
    ...
    fn threshold_is_user_configurable() {
        assert!(
            !App::source_is_dead(45, true, 0.0, false, 90),
            "90sn sabırda 45sn ölü sayılmamalı"
        );
        assert!(
            App::source_is_dead(90, true, 0.0, false, 90),
            "90sn sabırda eşikte ölü"
        );
    }
    ...
    fn loaded_source_is_never_killed_by_dead_check() {
        ...
        assert!(
            !App::source_is_dead(600, true, 0.0, true, 20),
            "yüklendiyse duration anlık 0 okunsa bile"
        );
    }
    ...
    fn socket_timeout_only_when_socket_never_seen() {
        assert!(
            App::socket_timeout_hit(26, false),
            "soket hiç gelmedi + 25sn doldu -> vazgeç"
        );
        assert!(!App::socket_timeout_hit(24, false), "henüz süre dolmadı");
        assert!(
            !App::socket_timeout_hit(600, true),
            "soket varken zaman aşımı uygulanmaz"
        );
    }
    ...
    fn anime4k_normal_maps_to_bundled_cnn_shader() {
        ...
        assert!(
            p.is_some(),
            "normal modu için CNN_x2_M shader'ı bundle edilmiş olmalı"
        );
    }
}
