use crate::api::{marathon_summary, Client, HistoryEntry, Settings, Title};
...
impl MarathonView {
    pub fn build(
        ...
        let stats_lbl = gtk::Label::new(Some(&format!("… / {} Anime Tamamlandı", total_count)));
        ...
        std::thread::spawn(move || {
            let fracs: Vec<f64> = summary_titles
                .iter()
                .map(|t| client_s.title_progress_frac(t))
                .collect();
        ...
        for (idx, item) in marathon_items.iter().enumerate() {
            ...
            chk.set_tooltip_text(Some(if item.completed {
                "Tamamlandı olarak işaretli"
            } else {
            ...
            let status_badge = gtk::Label::new(Some(if item.completed {
                "🏁 Tamamlandı"
            } else {
                "⏳ Devam Ediyor"
            }));
            status_badge.add_css_class(if item.completed {
            ...
            std::thread::spawn(move || {
                let _ = tx.send(client_c.title_progress_frac(&t_c));
            });
            let chk_u = chk.clone();
            ...
            glib::idle_add_local(move || match rx.try_recv() {
                Ok(frac) => {
                    ...
                    guard_u.set(false);
                    chk_u.set_tooltip_text(Some(if done {
                        "Tamamlandı olarak işaretli"
                    } else {
                        "Tamamlandı olarak işaretle"
                    }));
                    badge_u.set_text(if done {
                        "🏁 Tamamlandı"
                    } else {
                        "⏳ Devam Ediyor"
                    });
                    ...
                    badge_u.add_css_class(if done {
                        "status-badge-completed"
                    } else {
                        "status-badge-progress"
                    });
                    if done {
                        name_u.add_css_class("dim-label");
                    } else {
                        name_u.remove_css_class("dim-label");
                    }
                    glib::ControlFlow::Break
                }
                Err(_) => glib::ControlFlow::Continue,
...
impl SettingsView {
    pub fn build(
        ...
        search_toggle_row.set_subtitle(
        ...
        let current_ep_sc = ep_shortcuts
            .iter()
            .position(|&s| s == settings.quick_search_shortcut)
            .unwrap_or(0);
        ...
        let current_sc = search_shortcuts
            .iter()
            .position(|&s| s == settings.search_shortcut)
            .unwrap_or(0);
        ...
        let patience_adj = gtk::Adjustment::new(
            settings.source_patience_secs as f64,
            20.0,
            120.0,
            5.0,
            10.0,
...
• Proxy kapatılırsa uygulama otomatik normal bağlantıya döner, hiçbir ayarın bozulmaz.",
            ...
            vpn_status_row
                .set_subtitle("Yerel proxy (127.0.0.1:10808) üzerinden ISS kısıtlamalarını aşar");
            let refresh_vpn_status = {
                let row = vpn_status_row.clone();
                move || {
                    if crate::vpn::port_alive() {
                        row.set_subtitle(
                            "Çalışıyor — video trafiği 127.0.0.1:10808 üzerinden çıkıyor",
                        );
            ...
            vpn_btn_row
                .set_subtitle("ProtonVPN ücretsiz hesapla çalışır; kurulum için Başlat'a bas");
            ...
            stop_btn.connect_clicked(move |btn| {
                ...
                glib::spawn_future_local(async move {
                    ...
                    match killed {
                        Ok(true) => {}
                        Ok(false) => show_info_dialog(
                            parent.as_ref(),
                            "VPN Proxy",
                            "Çalışan sing-box süreci bulunamadı.",
                        ),
                        Err(_) => show_info_dialog(
                            parent.as_ref(),
                            "VPN Proxy Hatası",
                            "Proxy durdurulurken beklenmeyen bir hata oluştu.",
                        ),
                    }
                    rs2();
                });
        ...
        auto_update_row
            .set_subtitle("Başlatmada yeni sürümü kontrol eder ve AppImage'i kendisi günceller");
        ...
        ver_row.set_subtitle(&format!(
            "AnimeciX Masaüstü İstemcisi  •  v{}",
        ...
        let reinstall_btn =
            gtk::Button::with_label("Masaüstü Başlatıcısını Sistemime Kur / Güncelle");
...
pub fn build_shelf(
    title: &str,
    items: &[Title],
    make_card: Rc<dyn Fn(&Title) -> gtk::Box>,
) -> gtk::Box {
    ...
    shelf_title.set_margin_start(2);
    root.append(&shelf_title);
...

    {
        ...
        wheel.connect_scroll(move |_, _dx, dy| {
            ...
            let max = (hadj.upper() - hadj.page_size()).max(lower);
            let cur = hadj.value();
            let next = (cur + dy * 60.0).clamp(lower, max);
            if (next - cur).abs() < 0.5 {
    ...
    prev_btn.set_tooltip_text(Some("Geri kaydır"));
    {
        let hadj = scroll.hadjustment();
        prev_btn.connect_clicked(move |_| {
            let max = (hadj.upper() - hadj.page_size()).max(hadj.lower());
            hadj.set_value((hadj.value() - hadj.page_size() * 0.8).clamp(hadj.lower(), max));
        });
    }
    ...
    next_btn.set_tooltip_text(Some("İleri kaydır"));
    {
        let hadj = scroll.hadjustment();
...
fn shuffle_titles(mut v: Vec<Title>) -> Vec<Title> {
...
impl RecommendationsView {
    pub fn build(
        ...
        let title_lbl = gtk::Label::new(Some("✨ Sana Özel Öneriler"));
        title_lbl.add_css_class("title-2");
        title_lbl.set_xalign(0.0);
        title_lbl.set_hexpand(true);
        let count_lbl = gtk::Label::new(Some(""));
        count_lbl.add_css_class("dim-label");
        count_lbl.set_valign(gtk::Align::Center);
        let shuffle_btn = gtk::Button::with_label("Karıştır");
        shuffle_btn.add_css_class("pill");
        shuffle_btn.add_css_class("flat");
        shuffle_btn.set_valign(gtk::Align::Center);
        shuffle_btn.set_tooltip_text(Some("Önerileri yeniden karıştır"));
        header.append(&title_lbl);
        header.append(&count_lbl);
        header.append(&shuffle_btn);
        main.append(&header);
        ...
        let pool = Rc::new(RefCell::new(shuffle_titles(pool)));
        ...
        let append_batch = {
            ...
            Rc::new(move || {
                ...
                if idx.get() >= pool.len() {
                    let fresh = shuffle_titles(pool.clone());
                    *pool = fresh;
                    idx.set(0);
                }
                let start = idx.get();
                let end = (start + BATCH).min(pool.len());
                for t in &pool[start..end] {
                    flow.append(&make_card(t));
                }
                idx.set(end);
                let total = pool.len();
                count_lbl.set_text(&format!("{} / {} öneri", end.min(total), total));
            })
        };

        {
            let vadj = scroll.vadjustment();
            let append_batch = append_batch.clone();
            vadj.connect_value_changed(move |adj| {
                if adj.upper() - (adj.value() + adj.page_size()) < 600.0 {
                    append_batch();
                }
            });
        }

        {
            ...
            shuffle_btn.connect_clicked(move |_| {
                let mut cur = flow.first_child();
                while let Some(child) = cur {
                    let next = child.next_sibling();
                    flow.remove(&child);
                    cur = next;
                }
                let fresh = shuffle_titles(pool.borrow().clone());
                *pool.borrow_mut() = fresh;
                idx.set(0);
                append_batch();
                append_batch();
            });
    ...
    pub fn pool_from_cats(cats: &[crate::api::Category]) -> Vec<Title> {
