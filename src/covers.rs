...
impl CoverManager {
    ...
    pub fn thumb_url(url: &str, hd: bool) -> String {
        let t = if hd { "w500" } else { "w342" };
        url.replace(
            "image.tmdb.org/t/p/original",
            &format!("image.tmdb.org/t/p/{t}"),
        )
        .replace(
            "image.tmdb.org/t/p/w780",
            &format!("image.tmdb.org/t/p/{t}"),
        )
        .replace(
    ...
    fn finish_cover(&self, url: &str, bytes: Option<Vec<u8>>) {
        ...
        if let Some(b) = &bytes {
            ...
            for (key, pics) in waiters.iter() {
                let Some(rest) = key.strip_prefix(url) else {
                    continue;
                };
                let Some(rest) = rest.strip_prefix('@') else {
                    continue;
                };
                let Some((ws, hs)) = rest.split_once('x') else {
                    continue;
                };
                let (Ok(w), Ok(h)) = (ws.parse::<i32>(), hs.parse::<i32>()) else {
                    continue;
                };
